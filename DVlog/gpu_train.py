...
def train(
    ...
    train_model(model, train_dataloader, val_dataloader, config_dict, output_path, device)


def train_model(model, train_dataloader: DataLoader, val_dataloader: DataLoader, config_dict: ConfigDict, output_path: Path, device):
    ...
    for epoch in range(config_dict.epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # get the inputs and do the step forwards
            if config_dict.n_modalities == 1:
                # unpack the set for one modality
                inputs, labels = data[0].to(device), data[1].to(device)
            elif config_dict.n_modalities == 2:
                # more modalities so keep the tuple set
                inputs1, inputs2, labels = data[0].to(device), data[1].to(device), data[2].to(device)
